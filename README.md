# MatchLog Legal

This repository hosts the public legal pages for MatchLog using GitHub Pages.

## Suggested GitHub Pages URLs

Once GitHub Pages is enabled, the pages will be available at:

- `https://erndmrcn.github.io/matchlog-legal/`
- `https://erndmrcn.github.io/matchlog-legal/support/`
- `https://erndmrcn.github.io/matchlog-legal/privacy/`
- `https://erndmrcn.github.io/matchlog-legal/terms/`

These URLs are suitable for App Store support, privacy policy, and terms of use links.

## Files

- `index.html`: landing page with links to the legal documents
- `support/index.html`: MatchLog support page with FAQ and data deletion contact
- `privacy/index.html`: MatchLog Privacy Policy
- `terms/index.html`: MatchLog Terms of Use
- `assets/site.css`: shared styles for all pages

## GitHub Pages Setup

This repository includes a GitHub Actions workflow for Pages deployment.

1. Push this repository to GitHub.
2. Open repository `Settings`.
3. Go to `Pages`.
4. Under `Build and deployment`, set `Source` to `GitHub Actions`.
5. Wait for the `Deploy static content to Pages` workflow to complete after pushing to `main`.

Important:

- Do not use the `raw.githubusercontent.com` link for these files. Raw links show source code as text.
- Use the GitHub Pages domain instead, such as `https://erndmrcn.github.io/matchlog-legal/privacy/`.
